<x-app-layout>
                ...
                <div class=" flex-1 bg-gray-100 p-4 md:p-6" style="min-width: 50%">
                    <div class="container mx-auto">
                        ...
                        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
                            <h1 class="text-2xl font-semibold text-gray-700">Manage Faculty</h1>
                            <button class="add-faculty-btn w-full sm:w-auto bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">
                                Add Faculty
                            </button>
                        </div>
                        ...
                        <div class="bg-white rounded-lg shadow-md p-6">
                            <div class="dataTables_wrapper table-responsive">
                                <table id="faculty-tables" class="display w-full">
                                    <thead>
                                        <tr>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Action</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Faculty Name</th>
                                            ...
                                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Emp. ID</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">EPF/PPRN No.</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">LTC Availed</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Pay Level</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Basic Pay</th>
                                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Position Held</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($faculties as $faculty)
                                        <tr>
                                            <td class="px-6 py-4 whitespace-nowrap edit-staff-btn" data-id="{{ $faculty->faculty_id }}" title="Edit Faculty">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $faculty->fac_name }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $faculty->email }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $faculty->designation }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $faculty->empid }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $faculty->EPF_PPRN_No }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $faculty->ltc_availed }}</td>
    ...
    <script>
        $(document).ready(function () {
            const table = $('#faculty-tables').DataTable({
                responsive: true,
                scrollCollapse: true,
                pageLength: 25,
                order: [[1, 'asc']],
